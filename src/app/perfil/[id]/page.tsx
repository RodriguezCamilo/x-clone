    ...
    <section className="text-left flex flex-col h-auto w-full bg-black pt-2 border border-t-0 border-zinc-700">
      <div className="flex flex-row gap-4 px-4 pb-1">
        ...
        <div className="flex flex-col">
          ...
          <p className="font-light text-sm text-white/50">{manyPosts} posts</p>
        </div>
      </div>
      <div className="bg-zinc-600 h-52 w-full relative">
        {user.avatar_url 
        ? <img src={user.avatar_url} className="rounded-full size-32 absolute bottom-[-31%] left-4 border-4 border-black"></img>
      ...
      <div className="flex flex-row justify-end items-center w-full p-4">
        ...
        ? <button className="p-2 px-4 rounded-full font-semibold border border-zinc-700 transition hover:bg-white/10" >Editar perfil</button>
        : <button className="p-2 px-4 rounded-full font-semibold bg-white text-black hover:bg-white/95 transition">Seguir</button>
      }
      ...
      <div className="p-4">
        {user.bio 
        ? <p>{user.bio}</p>
        : <p>Este usuario no ha proporcionado informacion</p>}
      </div>
      ...
      <div className="grid grid-cols-3 w-full border border-x-0 border-t-0 border-zinc-700">
        <button className="py-4 font-semibold hover:bg-white/5 transition">Posts</button>
        <button className="py-4 font-semibold hover:bg-white/5 transition">Respuestas</button>
        <button className="py-4 font-semibold hover:bg-white/5 transition">Fotos y videos</button>
      </div>
      <main>
